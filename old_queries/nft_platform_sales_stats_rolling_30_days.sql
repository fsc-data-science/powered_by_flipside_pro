...
from 
  ...
  group by day_
;
--CREATE VIEW pro_charliemarketplace.ecosystem.nft_platform_sales_stats_rolling_30_days AS 
...
select date_trunc('day', block_timestamp) as day_, platform_name,
count(distinct tx_hash) as n_tx,
sum(n_ids) as n_nfts,
COALESCE(sum(case when nft_type = 'erc1155' then n_ids end),0) as n_erc1155,
sum(case when nft_type = 'erc721' then n_ids end) as n_erc721,
sum(case when event_type = 'bid_won' then 1 else 0 end) as n_bid_won,
sum(case when event_type = 'sale' then 1 else 0 end) as n_sale,
sum(price) as eth_sales_revenue,
